...
- Configure my nixos to use the pi-hole as the dns server (added a NAT to connect the board to the internet through the PC'c ethernet interface) inside a network.nix file that I added to the configuration.nix imports:
```nix
{config, pkgs, ...}:
